...
class ModelPolicy(torch.nn.Module):
    def __init__(self, inputs_count, hidden_count, outputs_count):
        ...
        self.output     = torch.nn.Linear(hidden_count, outputs_count)
...
class ModelCritic(torch.nn.Module):
    def __init__(self, inputs_count):
        ...
        self.hidden     = torch.nn.Linear(inputs_count, inputs_count)
        self.act        = torch.nn.ReLU()

        self.ext_value  = torch.nn.Linear(inputs_count, 1)
        self.int_value  = torch.nn.Linear(inputs_count, 1)
...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_shape, outputs_count):
        ...
        self.model_policy = ModelPolicy(448, 448, outputs_count)
        self.model_policy.to(self.device)

        self.model_critic = ModelCritic(448)
    ...
    def forward_features(self, state):
        y = self.model_features[0](state)
        y = self.model_features[1](y)
        y = self.model_features[2](y)
        y = self.model_features[3](y)
        y = self.model_features[4](y)
        y = self.model_features[5](y)
        y = self.model_features[6](y)
        y = self.model_features[7](y)
