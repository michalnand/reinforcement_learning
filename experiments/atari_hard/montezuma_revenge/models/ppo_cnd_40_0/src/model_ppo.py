...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_shape, outputs_count):
        ...
        self.model_actions = ModelPolicy(hidden_count, hidden_count, outputs_count)
        self.model_actions.to(self.device) 
    ...
    def forward_prediction(self, state_now, state_next):
        features_now    = self.model_features(state_now)
        features_next   = self.model_features(state_next)
        features        = features_next - features_now

        actions         = self.model_actions(features)
    ...
    def save(self, path):
        ...
        torch.save(self.model_actions.state_dict(), path + "model_actions.pt")
    ...
    def load(self, path):
        ...
        self.model_actions.load_state_dict(torch.load(path + "model_actions.pt", map_location = self.device))

        self.model_features.eval()  
        self.model_policy.eval() 
        self.model_critic.eval()
        self.model_actions.eval()
